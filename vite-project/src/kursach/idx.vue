...
function addToCart(product) {
  ...
  if (existing) {
    ...
    console.log('local.products:', local.products);

    // Если количество товара в корзине не превышает доступное количество на складе
    if (newQuantity <= availableQuantity) {
      existing.quantity = newQuantity;  // Увеличиваем количество товара в корзине
    } else {
      // Если количество товара в корзине больше, чем на складе
      console.warn(`Невозможно добавить больше ${availableQuantity} шт. товара "${product.name}" в корзину.`);
    }
  } else {
    // Если товара нет в корзине, добавляем его с количеством 1
    if (availableQuantity > 0) {
      local.cart.push({ ...product, quantity: 1 }); // Добавляем товар с количеством 1
      console.log('local.products:', local.products);
    } else {
